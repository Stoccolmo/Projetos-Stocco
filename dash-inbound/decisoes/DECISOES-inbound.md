...
### Deploy — AGUARDANDO
O classificador do modo auto bloqueou a escrita no editor do Apps Script (como em 27/08). Tudo está pronto em **`docs/aplicar-v21.js`** (33 hunks, cada âncora validada como única contra a produção atual; o script aborta sem alterar nada se alguma não casar 1×). Caminho: (1) Rodrigo cola `docs/aplicar-v21.js` no console do DevTools com o editor aberto (digitar `allow pasting` antes, se o Chrome pedir) → salvar **Index.html, Reader.gs e Cohort.gs** pelo ícone da UI → Implantar → Gerenciar implantações → Nova versão (**Versão 21**); (2) cola `docs/mrr_snapshot_2026-09-07.tsv` em A2 da aba "MRR por Pré-vendedor"; (3) abre a dash e clica "Atualizar dados" (regenera o Cohort com o roster certo e recarrega o snapshot de MRR). `docs/build-v21.js` regenera espelho + applier a partir dos hunks.
