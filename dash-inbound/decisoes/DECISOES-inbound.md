...
| 15 | **MRR** | **Snapshot de 10/08 sem data na tela**: agosto aparecia como mês fechado com Eduarda 6 reuniões/0 vendas, Giovanna 45/0 (real em 07/09: 72/10 e 155/14). | Snapshot **recalculado no Redshift** (mesma consulta: `entities.Deal` pipeline "Executivo de Vendas 2.0" × `reports.EventoAssinatura` Ativação; abr–set/26, 4 SDRs Inbound) e salvo em **`docs/mrr_snapshot_2026-09-07.tsv`** (49 linhas, inclui a linha de controle `snapshot | 2026-09-07`). **Ainda não está na planilha**: o classificador bloqueou a escrita automatizada dos ~100 valores — Rodrigo cola o TSV em A2 da aba "MRR por Pré-vendedor" (substitui as 40 linhas atuais). `Reader.gs` (V21) lê a linha `snapshot` e a dash mostra "snapshot de 07/09/2026 — meses iguais ou posteriores estão incompletos". `Number()||0` contra NaN. |
...
### Deploy — PUBLICADO: Versão 21 (07/09/2026, 16:14)
Rodrigo colou `docs/aplicar-v21.js` no console (retorno: `OK. Index.html: 26 hunks, 3729 linhas | Reader.gs: 4 hunks, 1197 linhas | Cohort.gs: 3 hunks, 319 linhas` — idêntico ao espelho). Salvar e implantar foram feitos pela UI (cliques não são bloqueados). Implantação ativa confirmada em "Gerenciar implantações": **Versão 21 em 7 de set. de 2026, 16:14**, código `AKfycbyI171A-...` inalterado (link do painel igual). Em seguida rodei `?action=refresh` com a janela de setembro: o Cohort saiu com **Eduarda 16 · Giovanna 32 · Pedro 28 · Vitória 23 = 99**, sem Luiz — igual ao HubSpot por pessoa. `metaInfo.feriados` veio com os 12 feriados da aba. `mrrPorPreVendedor.snapshot` veio `null` nesse primeiro refresh porque o TSV ainda não tinha sido colado — resolvido logo em seguida (ver abaixo).

Histórico do bloqueio: o classificador do modo auto barrou a escrita automatizada no editor e na planilha (como em 27/08). O que ficou pronto em **`docs/aplicar-v21.js`** (33 hunks, cada âncora validada como única contra a produção atual; o script aborta sem alterar nada se alguma não casar 1×). Tudo feito. Rodrigo colou o TSV em A2 (49 linhas) às 16:2x e rodei o refresh de novo: `mrrPorPreVendedor.snapshot = "2026-09-07"`, 24 linhas de agendamento + 24 de venda (abr–set), agosto agora com Eduarda 72/10, Giovanna 155/14, Pedro 119/15, Vitória 95/11, sem NaN. Cohort segue 99 com as 4 pessoas certas. **V21 completa em produção.** `docs/build-v21.js` regenera espelho + applier a partir dos hunks.

### 07/09/2026 (tarde) — Pergunta: Online vs Presencial 488 × Total do período 441 em agosto
Não é erro, são universos diferentes: Online vs Presencial conta **leads que entraram na etapa Agendado** no mês (Base Leads col E — mesmo número da etapa Agendado do Funil vs Meta); Visão Geral, Ranking, Cohort, overlay e MRR contam **passes pela data da reunião** (Neo Crescimento). Agosto: 488 (84/165/131/108) × 441 (72/155/119/95). Diferença = deslocamento de mês (agendado em ago, reunião em set), lead que não virou passe e reagendamento.

Propus unificar em "passe pela data da reunião" (base da meta/variável) migrando a aba Online vs Presencial e rotulando o Funil como "Agendado (entrada na etapa)". **Rodrigo aprovou a direção, mas adiou para uma próxima sessão (TO-DO)**: (1) migrar Online vs Presencial para a base dos passes (data da reunião + executivo do negócio; agosto passa de 488 para 441 e iguala o Ranking); (2) renomear a etapa do Funil para "Agendado (entrada na etapa)" com linha de referência "Reuniões no mês: N"; (3) frase de definição fixa no rodapé de todas as abas ("Reunião = passe com data da reunião no período; Validado = efetiva Sim"). Estimativa: uma sessão, backend em Reader.gs e possivelmente leads.gs, validar contra o HubSpot antes de publicar. Até lá o rótulo da aba (V21) explica o universo.

Lembrete operacional: "Atualizar dados" recarrega só o payload; código novo só entra com F5 na dash.
